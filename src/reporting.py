    ...
    def default(self, obj):
        ...
        try:
            return super(NpEncoder, self).default(obj)
             ...
             logging.warning(f"Object of type {type(obj)} is not JSON serializable. Representing as string.")
             return str(obj)


# --- Summary Generation ---

def generate_summary(analysis_results: dict, monthly_df_with_fv: pd.DataFrame, monthly_df_oos: pd.DataFrame) -> dict:
    """
    ...
    def safe_get(data_dict, key_list, default=None):
        current = data_dict
        for key in key_list:
            if isinstance(current, dict) or isinstance(current, tuple): # Allow tuples for shape
                try:
        ...
        if isinstance(current, (float, np.floating)) and (np.isnan(current) or np.isinf(current)):
             return default
        return current
    ...
    final_dict['ols_base_beta_active_pval'] = safe_get(ols_base, ['pvals_hac', 'log_active'])
    ...
    final_dict['ardl_cointegrated_5pct'] = safe_get(ardl, ['cointegrated_5pct']) # This might be None if p-value was nan
    final_dict['ardl_bounds_stat'] = safe_get(ardl, ['bounds_stat'])
    final_dict['ardl_bounds_p_lower'] = safe_get(ardl, ['bounds_p_lower']) # Get p-value too
    final_dict['ardl_ect_coeff'] = safe_get(ardl, ['ect_coeff'])

    final_dict['vecm_beta_active_coint'] = safe_get(vecm, ['beta_active_coint']) # LR beta from VECM
    final_dict['vecm_alpha_mcap'] = safe_get(vecm, ['alpha_mcap']) # Adjustment speed for mcap
    final_dict['vecm_alpha_mcap_p'] = safe_get(vecm, ['alpha_mcap_p'])
    final_dict['vecm_alpha_active_p'] = safe_get(vecm, ['alpha_active_p']) # P-value for active adjustment
    ...
    if not monthly_df_with_fv.empty:
        ...
        last_actual = monthly_df_with_fv['price_usd'].iloc[-1] if 'price_usd' in monthly_df_with_fv.columns else None
        last_fair_ext = monthly_df_with_fv['fair_price_ext'].iloc[-1] if 'fair_price_ext' in monthly_df_with_fv.columns else None

    # Use the dataframe that OOS modified to get last predicted price
    if not monthly_df_oos.empty:
         last_pred_oos = monthly_df_oos['predicted_price_oos'].iloc[-1] if 'predicted_price_oos' in monthly_df_oos.columns else None
    ...
    final_dict['last_fair_price_ext'] = last_fair_ext if pd.notna(last_fair_ext) else None
    final_dict['last_predicted_price_oos'] = last_pred_oos if pd.notna(last_pred_oos) else None
    ...
    beta_ext_sig = "significant" if isinstance(beta_ext_pval, (int, float)) and beta_ext_pval < 0.05 else "not significant"
    ...
    nasdaq_sig = "significant" if isinstance(nasdaq_pval, (int, float)) and nasdaq_pval < 0.05 else "not significant"
    nasdaq_pval_str = format_val(nasdaq_pval, precision=3, is_p_value=True)

    gas_pval = final_dict['ols_ext_pvals_hac'].get('log_gas')
    gas_sig = "significant" if isinstance(gas_pval, (int, float)) and gas_pval < 0.05 else "not significant"
    ...
    oos_mape_str = format_val(final_dict['oos_mape_percent'], precision=1) + "%" if final_dict['oos_mape_percent'] not in ["N/A", None] else "N/A"
    ...
    ardl_cointegrated = final_dict['ardl_cointegrated_5pct'] # Get raw value (could be None or False)
    ...
    if isinstance(last_actual_val, (int, float)) and isinstance(last_fair_val, (int, float)) and last_fair_val != 0:
