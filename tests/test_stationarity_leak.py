...
def test_stationarity_test_windowing():
    ...
    results_first_half_df = run_stationarity_tests(
        ...
        window_mask=first_half_mask
    )
    # Extract ADF p-value for 'value' column
    adf_p_first_half_str = results_first_half_df.set_index('series').loc['value', 'ADF p']
    ...
    results_second_half_df = run_stationarity_tests(
        df=df,
        cols_to_test=cols_to_test,
        window_mask=second_half_mask
    )
    adf_p_second_half_str = results_second_half_df.set_index('series').loc['value', 'ADF p']
