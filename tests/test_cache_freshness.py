...
def _dummy_cached_function(run_id: int) -> pd.DataFrame:
    ...
    return pd.DataFrame({"data": [run_id], "timestamp": [pd.Timestamp.now(tz=timezone.utc)]})
...
def test_cache_hit(manage_cache_dir):
    ...
    pd.testing.assert_frame_equal(result1, result2, check_dtype=False) # Timestamps might differ slightly if not cached perfectly
...
def test_cache_refresh_after_max_age(manage_cache_dir):
    ...
    past_mtime = current_time - max_age_seconds - 60 # Make it 1 minute older than max age
